...
public class BoardDao {
    ...
    * @throws SQLException
    */
   public int insert(BoardVO rec) throws BoardException, SQLException {
   ...
   public List<BoardVO> selectList() throws BoardException, SQLException {
      PreparedStatement ps = null;
      ResultSet rs = null;
   ...
   public BoardVO selectById(int seq) throws BoardException, SQLException {
      PreparedStatement ps = null;
      ResultSet rs = null;
      ...
      try {
         ...
         con = DriverManager.getConnection(dbUrl, dbUser, dbPass);
         // * sql 문장만들기
         String sql = "SELECT seq, title, writer, regdate, cnt, content FROM board_ex where seq =?";
         // * 전송객체 얻어오기
         ps = con.prepareStatement(sql);
         ps.setInt(1, seq);
         // * 전송하기
   ...
   // --------------------------------------------
   // ##### 게시글 보여줄 때 조회수 1 증가
   public void increaseReadCount(int seq) throws BoardException, SQLException {
   ...
   public int update(BoardVO rec) throws BoardException, SQLException {
      ...
      try {
         ...
         con = DriverManager.getConnection(dbUrl, dbUser, dbPass);
         // * sql 문장만들기
         String sql = "UPDATE board_ex set title = ?, content =? WHERE pass = ? AND seq = ?";
         // * 전송객체 얻어오기
         ps = con.prepareStatement(sql);
         ps.setString(1, rec.getTitle());
   ...
   // --------------------------------------------
   // ##### 게시글 삭제할 때
   public int delete(int seq, String pass) throws BoardException, SQLException {
