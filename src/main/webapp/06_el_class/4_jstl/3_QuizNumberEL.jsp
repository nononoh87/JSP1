...
<c:if test="${empty guess}">
	자~ 게임을 시작합시다
	<%
		int randNum =(int)(Math.random()*100) +1; //자바코딩밖에 없음.
...
<!--  (2) 다르다면 guess값이 answer값보다 크면 크다고 출력 작으면 작다고 출력 : gt = greater than, lt = less than -->
<c:if test="${not(empty guess)}">
	<c:choose>
		<c:when test="${guess eq answer}">정답입니다</c:when>
		<c:when test="${guess gt answer}">더작게</c:when>
		<c:when test="${guess lt answer}">더크게</c:when>		
	</c:choose>
</c:if>
